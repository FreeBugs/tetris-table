    ...
    if ((zeile+posy>=0) &&(zeile+posy<field_height))
    {
     if ( zeichen[zeile] & (1 << (7-spalte)) ) //if ((bitRead(zeile,A[spalte]))==true)
      { 
        ...
        if (((spalte+posx)<field_width) && ((spalte+posx)>=0) ) leds[Pos(spalte+posx, zeile+posy)] = CRGB(r,b, g); 
      }
      else    
      {
      if (((spalte+posx)<field_width) && ((spalte+posx)>=0) ) {leds[Pos(spalte+posx, zeile+posy)] = CRGB(0,0, 0);} 
      }
    }
   }
  }
